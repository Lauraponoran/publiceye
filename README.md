...
**Note on model checkpoints:** `models/trained_models/` includes the smaller `ViCCT_base_generic_1300_epochs.pth` checkpoint directly via Git LFS. The larger `Swin_ViCCT_large_22k_generic_1600_epochs.pth` checkpoint is hosted separately on Hugging Face due to its size — download it from [Lauraponoran/vicct-swin-large-22k](https://huggingface.co/Lauraponoran/vicct-swin-large-22k) and place it in `models/trained_models/` before setting `weights_path` to it.
